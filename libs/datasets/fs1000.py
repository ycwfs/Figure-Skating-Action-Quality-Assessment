...
class FineFS(Dataset):
    def __init__(
        ...
        if is_training:
            self.dict_list = self._load_json_db(os.path.join(annotation_folder, 'train_fs1000_new.txt'))
        else:
            self.dict_list = self._load_json_db(os.path.join(annotation_folder, 'val_fs1000_new.txt'))
