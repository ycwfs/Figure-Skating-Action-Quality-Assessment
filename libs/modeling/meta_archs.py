...
from .blocks import MaskedConv1D, Scale, LayerNorm, MaskedMHCross_CA
from .losses import ctr_diou_loss_1d, sigmoid_focal_loss, ctr_giou_loss_1d, mse_loss, l1_loss
...
class PCSScoreHead(nn.Module):
    ...
    def forward(self, fused_feats, fused_masks):
        for idx in range(len(self.head)):
            cur_out, _ = self.head[idx](fused_feats, fused_masks)
            cur_out = self.act(self.norm[idx](cur_out))
        cur_logits, _ = self.cls_head(cur_out, fused_masks)
...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.video_projector = nn.Linear(1024, 1024)
        ...
        self.score_head = PtTransformerScoreHead(
            fpn_dim, head_dim,
            kernel_size=head_kernel_size,
            num_layers=head_num_layers,
            with_ln=head_with_ln
        ...
        self.pcs_score_head = PCSScoreHead(
            fpn_dim, head_dim,
            kernel_size=head_kernel_size,
            num_layers=head_num_layers,
            with_ln=head_with_ln
    ...
    def project(self, video_list):
        for video in video_list:
            ...
            video['feats'] = self.video_projector(vf).transpose(0, 1)
            video['audio_feats'] = self.audio_projector(af).transpose(0, 1)
        return video_list
    ...
    def forward(self, video_list):
        ...
        batched_inputs, batched_audio_inputs, batched_masks = self.preprocessing(video_list)
        ...
        va_feats, audio_masks = self.audio_backbone(batched_audio_inputs, feats, batched_masks)
        va_fpn_feats, audio_fpn_masks = self.audio_neck(va_feats, audio_masks)
        for vm, am in zip(audio_masks, audio_fpn_masks):
            assert (vm == am).all(), "Video and audio fpn mask are not the same"
        ...
        if self.training:
            ...
            assert video_list[0]['element_scores'] is not None, "GT scores does not exist"
            ...
            # [[13,45],[78,23]]  [0.5,0.7]
            gt_cls_labels, gt_offsets, gt_scores = self.label_points(
                points, gt_segments, gt_labels, gt_element_scores)

            # compute the loss and return
            losses = self.losses(
                fpn_masks,
                out_cls_logits, out_offsets, out_scores,
                gt_cls_labels, gt_offsets, gt_scores, pcs_labels, out_pcs
            )
            return losses
            ...
            results = self.inference(
                ...
                out_cls_logits, out_offsets, out_scores, out_pcs
            )
            return results
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        if self.training:
            ...
            for audio_feat, pad_audio_feat in zip(audio_feats, batched_audio_inputs):
                pad_audio_feat[..., :audio_feat.shape[-1]].copy_(audio_feat)
        else:
        ...
        return batched_inputs, batched_audio_inputs, batched_masks
    ...
    def label_points(self, points, gt_segments, gt_labels, gt_element_scores):
        ...
        gt_cls, gt_offset, gt_scores = [], [], []

        # loop over each video sample
        for gt_segment, gt_label, gt_element_score in zip(gt_segments, gt_labels, gt_element_scores):
            cls_targets, reg_targets, score_target = self.label_points_single_video(
                concat_points, gt_segment, gt_label, gt_element_score
            )
            # append to list (len = # images, each of size FT x C)
            gt_cls.append(cls_targets)
            gt_offset.append(reg_targets)
            gt_scores.append(score_target)

        return gt_cls, gt_offset, gt_scores
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label, gt_element_scores):
        ...
        assert num_gts == num_score, "Number of gt labels and scores must be the same"
        ...
        if num_gts == 0:
            ...
            return cls_targets, reg_targets, score_targets
        ...
        for idx,x in enumerate(concat_points):
            abs_regress_range[idx,0] = x[0] - x[1]
            abs_regress_range[idx,1] = x[0] + x[2]
        ...
        for idx,element in enumerate(gt_segment):
            for i in range(num_pts):
                ...
                if concat_points[i][0] > element[0] and concat_points[i][0] < element[1]:
                    score_targets[i] = gt_element_scores[idx]
    ...
    def losses(
        self, fpn_masks,
        out_cls_logits, out_offsets, out_scores,
        gt_cls_labels, gt_offsets, gt_element_scores, pcs_labels, out_pcs
    ):
        ...
        score_loss = mse_loss(
            torch.cat(out_scores, dim=1)[pos_mask],
            gt_element_scores,
            reduction='sum'
        )
        score_loss /= self.loss_normalizer
    ...
    def inference(
        ...
        for idx, (vidx, fps, vlen, stride, nframes) in enumerate(
            ...
            results_per_vid = self.inference_single_video(
                ...
                cls_logits_per_vid, offsets_per_vid, scores_per_vid
            )
            ...
            results_per_vid['pcs'] = torch.sigmoid(out_pcs[idx])
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, out_score, pts_i, mask_i in zip(
                out_cls_logits, out_offsets, out_scores, points, fpn_masks
            ):
            ...
            pred_score_all.append(pred_score[keep_idxs2])

        # cat along the FPN levels (F N_i, C)
        segs_all, scores_all, cls_idxs_all, pred_score_all = [
            torch.cat(x) for x in [segs_all, scores_all, cls_idxs_all, pred_score_all]
        ]
        results = {'segments' : segs_all,
                   'scores'   : scores_all,
                   'labels'   : cls_idxs_all,
                   'pred_score' : pred_score_all}
    ...
    def postprocessing(self, results):
        ...
        for results_per_vid in results:
            ...
            if self.test_nms_method != 'none':
                ...
                segs, scores, labels, pred_scores = batched_nms(
                    segs, scores, labels,pred_scores,
